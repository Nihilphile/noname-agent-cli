...
function save(options,file=FILE){
  ...
  const gameSource=normalize(options.source),executable=path.resolve(options.executable || path.join(gameSource,'../..','无名杀.exe'));
...
module.exports={FILE,defaults,source,normalize,save};
