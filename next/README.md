...
当前版本：**1.3.0-experimental.7.1**（实验版）。[下载 Windows 使用包](https://github.com/Nihilphile/noname-agent-cli/releases/tag/v1.3.0-experimental.7.1)。

## 安装和第一次开局

需要 Windows、Node.js 22+，以及你自己安装的无名杀懒人包。当前实测基于子琪懒人包 v1.11.3；不同引擎版本尚未验证。隔离对局和本地房间还需要 Edge/Chrome，并沿用含 Nihilphile 扩展的配置。游戏本体、武将包和资源不随本工具分发。
...
无需 npm install。将下面示例目录换成自己的安装目录（包含“无名杀.exe”，也接受其 resources/app）：
...
新选择出现时重新观察。操作串遇到需要新决策、阶段变化或无法确认提交时会停止；不会自动重放。机器调用可加 --json。可选的 [noname-play skill](https://github.com/Nihilphile/noname-agent-cli/tree/main/skills) 提供游玩习惯说明。
...
- [验证范围与已知限制](docs/VALIDATION.md)、[本版更新](docs/RELEASE-experimental.7.1.md)
