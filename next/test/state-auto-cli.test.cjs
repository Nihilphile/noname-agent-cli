...
test('JSON keeps snapshot fields and does not consume the first text board', async t => {
  const f = fixture(t);
...
test('explicit state overrides remain authoritative and JSON never writes a receipt', async t => {
  const f = fixture(t);
...
test('observe, wait and act share own-turn cadence while off-turn responses always show', async t => {
  const f = fixture(t);
  await f.main(['observe']);
  ...
  assert.match(f.emitted[4], /自己 自己/); assert.match(f.emitted[5], /自己 自己/);
});
...
test('stale action and failed output do not consume the first display', async t => {
  ...
  await assert.rejects(f.main(['observe']), /output failed/);
  f.failPrint(false); await f.main(['observe']);
  assert.match(f.emitted[2], /自己 自己/);
});

test('extra phase, new epoch, bad receipt and unknown context safely show', async t => {
  const f = fixture(t);
  await f.main(['observe']);
  for (const [id, epoch] of [['extra', 'game'], ['p1', 'newgame']]) {
    f.set(f.make(id, epoch)); await f.main(['observe']);
    assert.match(f.emitted.at(-1), /自己 自己/);
  }
  fs.writeFileSync(path.join(f.dir, 'display-feedback.json'), '{broken');
  f.set(f.make('corrupt', 'third')); await f.main(['observe']);
  assert.match(f.emitted.at(-1), /自己 自己/);
  f.set(f.make(null, 'third')); await f.main(['observe']); await f.main(['observe']);
  assert.match(f.emitted.at(-2), /自己 自己/); assert.match(f.emitted.at(-1), /自己 自己/);
});
